...
int run_yang_parallel(SimRes * res, SimData * data, double dX, double dt, size_t J, size_t N, size_t NT, size_t NE) {
    ...
    for (unsigned n=data->timeskip+1; n<NE; ++n) {
        ...
        if (!flagDone && n < N) {
            ...
            update_thermal_values_yang_parallel(alpha_n, kappa_n, c_n, rho_seg_n, R_seg_n, T_curr, J, A, B, gamma, data->T_c, I0[n-1], data->I_c0, data->rho_norm_std, data->c_p, data->T_ref_std, R_seg);
