...
int calculate_bias_from_target_currents_wtf_2s(double * v_I_b0, double * v_I_b1, double I_t0, double I_t1, double R_s0, double R_s1, double R_01, double R_p0, double R_p1) {
    // set up matrix and vector for Ax = b calculation
    lapack_int n, nrhs, info;
    n = 2; nrhs = 1;
...
int run_waterfall_2s_res(SimRes * res, SimData * data, double dX0, double dX1, double dt, size_t J0, size_t J1, size_t N, size_t NE, size_t NTL) {
    ...
    if (fabs(data->I_b0_wtf) < 1e-12 && fabs(data->I_b1_wtf) < 1e-12) {
        calculate_bias_from_target_currents_wtf_2s(&v_I_b0, &v_I_b1, data->I_t0_wtf, data->I_t1_wtf, data->R_s0_wtf, data->R_s1_wtf, data->R_01_wtf, data->R_p0_wtf, data->R_p1_wtf);
    } else {
