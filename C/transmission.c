...
int advance_time_transmission_constRr(double * I_tl_np1, double * V_c_tl_np1, double * I_tl_n, double * V_c_tl_n, double * A, double * b, lapack_int * ipiv, size_t NTL, double XT, double YT, double R_L, double I_b_tl_np1, double I_b_tl_n) {
    // set up matrix and vector for Ax = b calculation
    lapack_int info;
    ...
    // update rhs of equation
    b[0] = (XT-R_r)*I_tl_n[0] + V_c_tl_n[0] + XT*(I_b_tl_np1 - I_b_tl_n);
...
int sim_transmission_line(SimData * data, SimRes * res, size_t NE, size_t NTL, size_t numberOfIb) {
    ...
    if (data->simTL == 1) {
        ...
        for (unsigned n=data->timeskip+1; n<NE; ++n) {
            ...
            advance_time_transmission_varRr(I_tl_curr, V_c_tl_curr, I_tl_prev, V_c_tl_prev, AT_tmp, AT, bT, ipiv, NTL, XT, YT, data->R_L_std, R_r, v_I);
        ...
        for (unsigned n=data->timeskip+1; n<NE; ++n) {
            ...
            I_b_tl_curr = 2*(v_I - Isum_n);
            advance_time_transmission_constRr(I_tl_curr, V_c_tl_curr, I_tl_prev, V_c_tl_prev, AT, bT, ipiv, NTL, XT, YT, data->R_L_std, I_b_tl_curr, I_b_tl_prev);
            Itlsum[n] = subsum_vector(I_tl_curr, 1, NTL+1);
            Itl[n] = 0;
            Iload[n] = I_b_tl_curr - I_tl_curr[0] - Itlsum[n];
