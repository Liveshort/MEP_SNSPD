...
// function that fills up the static part of the transmission line matrix
int fill_transmission_matrix(double * A, double * b, size_t NT, double XT, double YT, double R_L) {
    unsigned rl = 2*NT+1;
...
// advances the transmission model one timestep using BLAS matrix algebra.
// you need the LAPACK for this, or work out the matrix logic on your own (not recommended)
int advance_time_trns_3_wtf_res_par(double * A, double * b, double * I_trns_np1, double * V_trns_np1, double * I_snspd_back, double I_trns_n, double V_trns_n, double I_snspd, double I_b, size_t NT, double XT, double YT, double R_L) {
...
int run_three_stage_waterfall_res_par(SimRes * res, SimData * data, double dX0, double dX1, double dX2, double dt, size_t J0, size_t J1, size_t J2, size_t N, size_t NT, size_t NE) {
    ...
    // pre-allocate space for the transmission line matrix
    size_t NT = 10;
    double * AT = calloc(NT*NT, sizeof(double));
    double * bT = calloc(NT, sizeof(double));

    // fill the static part of the transmission line matrix
    fill_transmission_matrix(AT, bT, NT, XT, YT, data->R_L_wtf);
    ...
    for (unsigned n=data->timeskip+1; n<NE; ++n) {
        ...
        advance_time_electric_3_wtf_res_par(&I0[n], &I1[n], &I2[n], &I3[n], &I4[n], &I5[n], &V_c[n], I0[n-1], I1[n-1], I2[n-1], I3[n-1], I4[n-1], I5[n-1], V_c[n-1], XW0, XP0, XW1, XP1, XW2, XP2, X01, XM, YM, R0[n-1], R0[n], R1[n-1], R1[n], R2[n-1], R2[n], data->R_L_wtf, data->R_01_wtf, data->R_12_wtf, data->R_small_wtf, data->R_s0_wtf, data->R_s1_wtf, data->R_s2_wtf, data->R_p0_wtf, data->R_p1_wtf, data->R_p2_wtf, v_I_b0, v_I_b1, v_I_b2);
