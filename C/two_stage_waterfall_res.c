...
static inline double I_cT_2_wtf_res(double I_c0, double T, double T_c) {
...
int update_thermal_values_2_wtf_res(double * alpha_n, double * kappa_n, double * c_n, double * rho_seg_n, double * R_seg_n, double * T_n, size_t J, double A, double B, double gamma, double T_c, double I_n, double I_c0, double rho_norm, double c_p, double T_ref, double R_seg) {
    ...
    for (unsigned j=0; j<J; ++j) {
        ...
        if (T_n[j] > T_c || I_n > I_cT_2_wtf_res(I_c0, T_n[j], T_c)) {
...
int run_two_stage_waterfall_res(SimRes * res, SimData * data, double dX0, double dX1, double dt, size_t J0, size_t J1, size_t N, size_t NT, size_t NE) {
    ...
    for (unsigned n=data->timeskip+1; n<NE; ++n) {
        ...
        if (!flagDone && n < N) {
            ...
            update_thermal_values_2_wtf_res(alpha0_n, kappa0_n, c0_n, rho_seg0_n, R_seg0_n, T0_curr, J0, A, B, gamma, data->T_c, I0[n-1], data->I_c0_wtf, data->rho_norm_wtf, data->c_p, data->T_ref_wtf, R_seg0);
            update_thermal_values_2_wtf_res(alpha1_n, kappa1_n, c1_n, rho_seg1_n, R_seg1_n, T1_curr, J1, A, B, gamma, data->T_c, I1[n-1], data->I_c1_wtf, data->rho_norm_wtf/surfaceRatio10, data->c_p, data->T_ref_wtf, R_seg1);
